...
class AdaBoostClassifier():
    def __init__(self, base_estimator, n_estimators=3): # Optional Arguments: Type of estimator
        ...
        self.estimators = [self.baseEstimator(max_depth = 1, criterion = 'entropy') for _ in range(n_estimators)]
    ...
    def fit(self, X, y):
        ...
        for i, estimator in enumerate(self.estimators):
            ...
            alpham = 0.5 * np.log((1 - error)/error)

            alphas.append(alpham)
            wrong = predictions != y
            correct = predictions == y
            weights[wrong] = weights[wrong] * np.exp(alpham)
            weights[correct] = weights[correct] * np.exp(-alpham)
    ...
    def predict(self, X):
        ...
        self.testX = X
        preds = np.array([estimator.predict(X) for estimator in self.estimators])*2 - 1
        y = pd.Series(((np.sign(np.dot(preds.T, self.alphas)) + 1)/2).astype(int))
    ...
    def plot(self):
        ...
        for i in range(self.nEstimators):
            ...
            plt.title(f'Estimator {i}, alpha = {self.alphas[i]}')
            weights = 20 * 2**( 2*(self.weights[i])/max(self.weights[i]))
            self.plotOneEstimator(ax, self.estimators[i].predict, xx, yy, X_train, y_train, weights)
